...
public class BaseMovementController : MonoBehaviour
{
    ...
    public int FacingDirection {
        ...
        private set {
            ...
            if (m_FacingDirection != oldFacing) {
                this.FacingFlipped(m_FacingDirection);
            }
        }
    }
    ...
    public BaseDashModule DashModule {
        get { return m_Dash.Module; }
        private set { m_Dash.Module = value; }
    }

    public BaseActionModule ActionModule {
        get { return m_CustomAction.Module; }
        private set { m_CustomAction.Module = value; }
    }
    ...
    public float MovementSpeed {
        get { return m_MovementSettings.Speed; }
        set { m_MovementSettings.Speed = value; }
    }
...
#region Monobehaviour
    ...
    public bool IsInAir()
    {
        return !m_Motor.Collisions.Below && !m_Motor.Collisions.Left &&
               !m_Motor.Collisions.Right; //IsState(MotorState.Jumping) || IsState(MotorState.Falling);
    }
    ...
    public void _Update(float timeStep)
    {
        ...
        // on ladder state
            ...
            // Second pass check
            if (m_OnLadderState.HasRestrictedArea) {
                targetPos.x = Mathf.Clamp(targetPos.x, m_OnLadderState.RestrictedAreaBottomLeft.x,
                    m_OnLadderState.RestrictedAreaTopRight.x);
                targetPos.y = Mathf.Clamp(targetPos.y, m_OnLadderState.RestrictedAreaBottomLeft.y,
                    m_OnLadderState.RestrictedAreaTopRight.y);
        ...
        else // other state
        {
            ...
            if (IsOnGround()) {
                ...
                m_Velocity.x = Mathf.SmoothDamp(m_Velocity.x, targetVecX, ref m_VelocityXSmoothing,
                    m_MovementSettings.AccelerationTimeAirborne);
            }
            /*
            m_Velocity.x = Mathf.SmoothDamp(m_Velocity.x, targetVecX, ref m_VelocityXSmoothing,
    ...
    private void updateTimers(float timeStep)
    {
        ...
        if (m_FallingJumpPaddingFrame >= 0)
            m_FallingJumpPaddingFrame--;
        if (m_WillJumpPaddingFrame >= 0)
            m_WillJumpPaddingFrame--;
    }
    ...
    private void changeState(MotorState state)
    {
        ...
        if (IsState(MotorState.WallSliding)) {
            this.WallSliding.Invoke(m_WallInteraction.WallDirX);
        }
